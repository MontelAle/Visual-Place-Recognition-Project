...
This script automates a four-stage evaluation flow:

- Run `VPR-methods-evaluation/main.py` to compute retrieval predictions
  (descriptors / top-k predictions) for the configured datasets. When
  invoked with `--save_for_uncertainty` this step also saves a
  `z_data.torch` file containing `database_utms`, `predictions` and
  `distances` required by the uncertainty evaluation.
...
- Run the uncertainty evaluation (`vpr_uncertainty.eval`) which loads the
  previously saved `z_data.torch` and the produced `preds`/`inliers` to
  compute uncertainty metrics (L2/PA/SUE/Inliers AUCs).
...
def main(dry_run=False, method=None, backbone=None, descriptors_dimension=None, dataset=None):
    ...
    for ds_name, paths in datasets.items():
        ...
        for matcher in matchers:
            ...
            # 4) Run uncertainty evaluation using the same preds/inliers and the z_data saved
            #    by the VPR evaluation (z_data.torch in the same timestamped logs folder).
            z_data_path = latest / "z_data.torch"
            if not z_data_path.exists():
                print(f"WARNING: z_data file not found at {z_data_path}. Skipping uncertainty eval.")
            else:
                cmd_uncertainty = (
                    f"{py} -m vpr_uncertainty.eval "
                    f"--preds-dir '{preds_dir}' "
                    f"--inliers-dir '{inliers_dir}' "
                    f"--z-data-path '{z_data_path}'"
                )
                sh(cmd_uncertainty, dry_run=dry_run)
